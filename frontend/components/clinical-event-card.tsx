...
const eventTypeLabels: Record<string, { label: string; color: string }> = {
  ...
  'lab-test': { label: 'Resultado de Laboratorio', color: 'bg-purple-50 text-purple-700 border border-purple-200' },
  imaging: { label: 'Reporte de Imagenología', color: 'bg-yellow-50 text-yellow-700 border border-yellow-200' },
  procedure: { label: 'Procedimiento', color: 'bg-pink-50 text-pink-700 border border-pink-200' },
  vaccination: { label: 'Vacunación', color: 'bg-primary/10 text-primary border border-primary/30' },
...
export function ClinicalEventCard({
  ...
  return (
    <Card
      className="hover:shadow-lg transition-shadow cursor-pointer gap-2 h-full flex flex-col"
      onClick={onClick}
    >
      ...
      <CardContent className="space-y-3 flex-1 flex flex-col">
        ...
        <div className="pt-3 border-t border-border flex items-center justify-between mt-auto">
          {event.stellarTxHash ? (
            <button
              type="button"
              className="text-xs bg-green-100 text-green-800 border border-green-200 px-2 py-1 rounded hover:bg-green-200 transition-colors"
              onClick={(e) => {
                e.stopPropagation();
                window.open(`https://stellar.expert/explorer/testnet/tx/${event.stellarTxHash}`, '_blank', 'noopener,noreferrer');
              }}
            >
              Verificado
            </button>
